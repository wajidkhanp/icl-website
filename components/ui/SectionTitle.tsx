...
export default function SectionTitle({
  ...
  return (
    <div className={`mb-8 ${center ? "text-center" : ""}`} data-aos="fade-up">
      {badge && (
      ...
      <h2 className="font-cinzel font-bold text-3xl md:text-4xl text-islamic-700 mb-4 leading-tight">
        {title}
        {titleHighlight && (
          <span className="text-islamic-600"> {titleHighlight}</span>
        )}
      </h2>
      {subtitle && (
        <p className={`text-gray-600 text-base md:text-lg leading-relaxed ${center ? "max-w-2xl mx-auto" : "max-w-2xl"}`}>
          {subtitle}
        </p>
      )}
    </div>
